...
public class Bullet : MonoBehaviour {
    ...
    /// </summary>
    public float frequencyIncrement = 0.05f;

    /// <summary>
    /// sinewave-like movement components
    /// </summary>
    public float sineX, sineY = 0f;
    ...
    {
        return Random.Range(0f, 100f) < ricochetChancePercent;
    }
    ...
    void Update()
    {
        ...
        if (gameObject != null)
        {
            ...
            if (amplitude > 0f)
            {
                ...
                var beta = directionAngle + Mathf.PI / 2;
                frequency += frequencyIncrement;
                var distance = amplitude * Mathf.Sin(frequency);
                var x = bulletXPosition + Mathf.Cos(beta) * distance;
                var y = bulletYPosition + Mathf.Sin(beta) * distance;
